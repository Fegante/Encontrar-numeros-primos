...
void verif_viabilidade_macrobloco() {
	if ((LINHA * COLUNA) % (L_MACRO * C_MACRO) != 0) {
		printf("Essa estrutura de matriz e macrobloco n�o � compativel\n");
	}
}
...
{ // cria a matriz **testada**
	...
	if (matriz == NULL)
	{ // verifica se ainda tem espa�o na memoria
	...
	{ // percorre as linhas da matriz
		...
		{                                                  // percorre as coluna da matriz
			matriz[m][n] = (rand() % MAX_VALOR_ALEATORIO); // gera um numero entre 0  a 29999 e coloca na posi��o Lin x Col
		}
	}
...
{             // destroi a matriz **testada**
	int i;
	int	j=0; // variavel axiliar;
	if (matriz == NULL)
	{ // verifica se a matriz n�o � nula
		return (NULL);
	}
	if (LINHA < 1 || COLUNA < 1)
...
void percorre_macrobloco(void* arg)
{
	...
	num_primos = local_primo + num_primos; // Regi�o critica.
